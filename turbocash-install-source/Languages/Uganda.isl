; *** Inno Setup version 6.3.3+ Uganda English (en-UG) messages ***
; Tailored for the Ugandan market - 262 lines - Source: TurboCASH 2026 Reference - Regional Version (UG)

[LangOptions]
LanguageName=English (Uganda)
LanguageID=$0409
...
SelectTasksLabel2=Select the additional tasks the wizard should perform while installing TurboCASH5-4, and click Next.
...
BadGroupName=The folder name cannot contain the following following characters:%n%n%1
...
PreviousInstallNotCompleted=The previous installation or uninstallation was not completed. You must restart your computer to complete that process.%n%nAfter restarting, run the Setup Wizard again to complete the installation of [name].
...
comp_firebird_local=Install local Firebird database components (required for local use of [name])
comp_firebird_server=Install Firebird database server (required for TurboCASH network or client-server operations)
comp_flamerobin=Install FlameRobin version 0.9.3 (SQL browser) (Optional)

; === Post Installation Launch Application ===
run_launch_brand=Run %1
...
TasksGroupDescription_Unicode=Font and Unicode Settings:%nFor reading legacy TurboCASH databases. If unsure, consult a specialist.
