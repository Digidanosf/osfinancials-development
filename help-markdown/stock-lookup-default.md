...
The standard "*Lookup*" screen is as follows:

![Image](<lib/lookup-stock-items-standard.png>)
...
| ![Image](<lib/admon-tip.png>) | *The **Auto-select** tick box will only be displayed when the* ![Image](<lib/lookup-use-filter.png>) *(right-click) is selected (ticked). The "Auto-select" option allows you to enter any keys to filter by stock code, description, barcode, etc.* |
| --- | --- |
...
1) &nbsp;
   ...
   1) **P2:** This displays the "*Selling price 2*" field of the Selling prices section on the [**Ledger**](<stock-item-tab-ledger.md>) tab of **Stock items** (**Default** ribbon). The price is displayed as exclusive of tax (VAT/GST/Sales tax) and the price is displayed as inclusive of tax (VAT/GST/Sales tax) between brackets.&nbsp;
...
| ![Image](<lib/admon-note.png>) | *The following options is not available on the Stock lookup in when selecting stock items on Reports:* ***Stock group 2** - Created / added in [**Groups***](<stock-item-reporting-groups.md>) *- **Stock group 2** on the [**Setup***](<setup-ribbon.md>) *ribbon and linked to the stock item.* ***Multilang** - Multi-language stock item descriptions.* |
| --- | --- |
