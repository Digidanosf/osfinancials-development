...
| ![Image](<lib/admon-note.png>) | *Orders cannot be updated (posted) to the ledger and it generates no transactions. But, once the Order has been converted to a Purchase document, the Purchase document can be edited (if necessary) and updated (posted) to the ledger.* |
| --- | --- |


| ![Image](<lib/admon-tip.png>) | *The following options is available to convert Orders to Purchases:* [***Confirm quotes and orders***](<doc-grid-confirm-orders.md>) *context menu option on the "Document entry" list screen for the Orders document type.*&nbsp; [***Input → Confirm***](<confirm-orders.md>) *menu.*&nbsp; |
| --- | --- |


| ![Image](<lib/admon-tip.png>) | ***Options to create / edit orders:*** ***New** - On the “Orders” screen, click the **New** button. Select a creditor (supplier / vendor) account on the "Creditor accounts" screen.* ***Edit** - On the “Orders” screen, click the **Edit** button.* ***Copy** - "**Documents → Copy document**" or "**Documents → Copy to → Orders**" option on context menu (right-click).* *On "Invoices" screen, select "**Documents → [Create purchase on default supplier 1***](<doc-grid-purchase-def-supplier.md>)*" option on context menu (right-click).* &nbsp; |
| --- | --- |
...
# Unposted Order entry form

The sections and fields of the Order screen, is as follows:

![Image](<lib/document-entry-order.png>)
...
1) &nbsp;
   ...
   1) **Salesperson** - Select a Salesperson. Salespersons are optional for orders. This field is to show who handled the order. The salespersons are used for reports and to filter and search documents.&nbsp;
   ...
   1) **Message** - The message, as entered in **Setup →** **Documents setup - [Orders**](<documents-setup-orders.md>) tab (**Setup** ribbon) will be displayed. You may edit / enter messages to be printed on the document layout file.&nbsp;
...
![Image](<lib/document-entry-order-printed.png>)
